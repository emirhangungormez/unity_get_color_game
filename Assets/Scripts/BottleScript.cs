...
public class BottleScript : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (IsFull() && IsSameColor() && !isSelectionInProgress && !sheepMoved && transform.parent.name == "BottleHolder")
        {
            ...
            foreach (GameObject obj in sheepsInHolder)
            {
                ...
                if (int.TryParse(obj.tag, out tagNumber)) // GameObject'in tag'ini integer'a dönüştür
                {
                    if (selectedBottleColorIndex == tagNumber) // Liste elemanı ile tag sayısı eşleşirse
                    {
                        ...
                        // Koyunun konumunu al
                        Transform sheepTransform = obj.transform.GetChild(1); // "holder" transform'u alınır;
                        Vector3 sheepPosition = sheepTransform.transform.position;
    ...
    private IEnumerator MoveToSheep(Vector3 targetPosition, GameObject sheep)
    {
        ...
        float waitTimeBeforeMove = 1f; // Hareket etmeden önce bekleme süresi
        ...
        if (sheepsBottleObject != null)
        {
            ...
            if (sheep != null)
            {
                ...
                // Animator'u bul
                Transform hoofTransform = sheep.transform.GetChild(0);
                GameObject hoofGameobject = hoofTransform.gameObject;
                Animator assignedAnimator = hoofGameobject.GetComponent<Animator>(); // Atanmış objenin Animator bileşeni
                assignedAnimator.enabled = true;
    ...
    private Vector3 CalculateTargetPos(Vector3 targetPosition)
    {
        ...
        {
            return targetPosition + new Vector3(-0.85f, 1.2f, 0f);
        }
        else
        {
            return targetPosition + new Vector3(0.85f, 1.2f, 0f);
    ...
    private void AnimateLiquidTransfer()
    {
        ...
        else if (animFrame < 150 + liquidTransferCount * 90)
        {
            ...
            float smoothFrame = Mathf.SmoothStep(0, 1, curFrame);

            transform.eulerAngles = new Vector3(0, 0, rotIncrement) * rotMultiplier;
            transform.position = Vector3.Lerp(targetPos, startPos, smoothFrame);
