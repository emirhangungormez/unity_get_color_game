...
public class LevelCreatorScript : MonoBehaviour
{
    ...
    {
        float yOffset = 1.85f; // Y ekseni üzerindeki başlangıç boşluğu
        float gapBetweenBottle = 1.8f; // Şişeler arasındaki dikey boşluk
        ...
        for (int groupIndex = 0; groupIndex < groupCount; groupIndex++)
        {
            ...
            for (int i = 0; i < bottlesInThisGroup; i++)
            {
                ...
                float yPos = yOffset - groupIndex * gapBetweenBottle;

                GameObject bottle = Instantiate(bottlePrefab, bottleHolder.transform);
                bottle.transform.position = new Vector3(xPos * 0.8f, yPos); // Şişeler arasındaki dikey boşluk
